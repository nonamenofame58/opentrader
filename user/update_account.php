      ...
      if (isset($_POST['update'])) {
          ...
          if (!empty($current_password)) {
            if (!empty($password) && !empty($password_2)) {
              if ($password != $password_2) {
                die("<script>window.location.href ='index.php?passwordNotMatch=notMatch'</script>");
              }

              if (!checkCurrentPassword($username,$email,$current_password)) {
                die("<script>window.location.href ='index.php?currentPasswordWrong=wrong'</script>");
              }
            } else {
              die("<script>window.location.href ='index.php?enterNewPassword=enter'</script>");
            }
          } else {
            die("<script>window.location.href ='index.php?enterCurrentPassword=enter'</script>");
          }
        }
        ...
        if ($stmt->execute()) {
          ...
          if ($stmt->affected_rows > 0) {
            $_SESSION['username'] = $username;
            $_SESSION['email'] = $email;
            echo('<script>window.location.href ="index.php?accountUpdate=updated" </script>');
          } else {
